...
class Malla extends Connection
{
    ...
    public function Registrar($unidades)
    {
        ...
        if (!$this->Existecodigo()) {
            if (!$this->ExisteCohorte()) {
                try {
                    $co->beginTransaction();
                    $stmt = $co->prepare("INSERT INTO tbl_malla( mal_codigo, mal_nombre, mal_descripcion, mal_cohorte, mal_estado) VALUES (:mal_codigo, :mal_nombre, :mal_descripcion, :mal_cohorte, 1)");
                ...
                $r['resultado'] = 'existe';
                $r['mensaje'] = '¡Atención!<br>La cohorte de la malla ya existe'; // nombre
            }
        } else {
            $r['resultado'] = 'existe';
    ...
    public function Eliminar(){
        ...
        $r = array();
        try {
            $stmt = $co->prepare("UPDATE tbl_malla SET mal_estado = 0 WHERE mal_id = :mal_id and mal_estado = 1");
            $stmt->bindParam(':mal_id', $this->mal_id, PDO::PARAM_INT);
            $stmt->execute();
