...
class Malla extends Connection
{
    ...
    public function __construct($mal_id = null, $mal_codigo = null, $mal_nombre = null, $mal_anio = null, $mal_cohorte = null, $mal_descripcion = null, $mal_estado = null, $mal_activa = null)
    {
        parent::__construct();
        $this->mal_id = $mal_id;
    ...
    public function activarMalla() {
        ...
        try {
            ...
            $stmt_activar = $co->prepare("UPDATE tbl_malla SET mal_activa = 1 WHERE mal_id = :mal_id");
            $stmt_activar->bindParam(':mal_id', $this->mal_id, PDO::PARAM_INT);
    ...
    public function Modificar($unidades){
        ...
        if (!$this->Existecodigo()) {
            if (!$this->ExisteCohorte()) {
                try {
                    $co->beginTransaction();
                    $stmt = $co->prepare("UPDATE tbl_malla SET mal_cohorte = :mal_cohorte, mal_codigo = :mal_codigo, mal_nombre = :mal_nombre,  mal_descripcion = :mal_descripcion WHERE mal_id = :mal_id AND mal_estado = 1");
                    $stmt->bindParam(':mal_cohorte', $this->mal_cohorte, PDO::PARAM_STR);
                    $stmt->bindParam(':mal_codigo', $this->mal_codigo, PDO::PARAM_STR);
                    $stmt->bindParam(':mal_nombre', $this->mal_nombre, PDO::PARAM_STR);
                    $stmt->bindParam(':mal_descripcion', $this->mal_descripcion, PDO::PARAM_STR);
    ...
    public function Eliminar(){
        ...
        $r = array();
        try {
            $stmt = $co->prepare("UPDATE tbl_malla SET mal_estado = 0, mal_activa = 0 WHERE mal_id = :mal_id");
            $stmt->bindParam(':mal_id', $this->mal_id, PDO::PARAM_INT);
            $stmt->execute();
