...
class Malla extends Connection
{
    ...
    public function Registrar($unidades)
    {
        ...
        try {
            ...
            if (!empty($trayectosFaltantes)) {
                $nombresFaltantes = array_map(function ($t) {
                    return $t == '0' ? 'Inicial' : 'Trayecto ' . $t;
                }, $trayectosFaltantes);
                return [
                    'resultado' => 'error',
                    'mensaje' => 'Error de validación del servidor: Debe incluir al menos una UC de los trayectos: ' . implode(', ', $nombresFaltantes)
                ];
            }
        } catch (Exception $e) {
    ...
    public function Modificar($unidades)
    {
        ...
        try {
            ...
            $stmtDelUC = $co->prepare("DELETE FROM uc_malla WHERE mal_codigo IN (SELECT mal_codigo FROM (SELECT mal_codigo FROM tbl_malla WHERE mal_cohorte = :mal_cohorte AND mal_estado = 0) AS t)");
            $stmtDelUC->bindParam(':mal_cohorte', $this->mal_cohorte, PDO::PARAM_INT);
            $stmtDelUC->execute();
            $stmtDelMalla = $co->prepare("DELETE FROM tbl_malla WHERE mal_cohorte = :mal_cohorte AND mal_estado = 0");
            $stmtDelMalla->bindParam(':mal_cohorte', $this->mal_cohorte, PDO::PARAM_INT);
            $stmtDelMalla->execute();
