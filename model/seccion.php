...
class seccion extends Connection
{
    ...
    private function determinarFaseActual() {
        try {
            $stmt = $this->Con()->prepare("SELECT ani_apertura_fase1, ani_cierre_fase1, ani_apertura_fase2, ani_cierre_fase2 FROM tbl_anio WHERE ani_activo = 1 AND ani_estado = 1 LIMIT 1");
            $stmt->execute();
            ...
            if ($fechas) {
                ...
                $cierre_f1 = new DateTime($fechas['ani_cierre_fase1']);
                $cierre_f1->setTime(23, 59, 59);
    
                $apertura_f2 = new DateTime($fechas['ani_apertura_fase2']);
                $cierre_f2 = new DateTime($fechas['ani_cierre_fase2']);
    ...
    public function EjecutarPromocionAutomatica() {
        ...
        try {
            ...
            $stmt_secciones_f1 = $co->prepare("SELECT sec_id, sec_codigo FROM tbl_seccion WHERE ani_id = :ani_id AND sec_estado = 1 AND sec_codigo LIKE '_1%'");
            ...
            foreach ($secciones_origen as $origen) {
                ...
                try {
                    ...
                    $nuevas_clases = [];
                    $stmt_uc_fase2 = $co->prepare("SELECT u.uc_id FROM tbl_uc u INNER JOIN uc_docente ud ON u.uc_id = ud.uc_id WHERE ud.doc_id = :doc_id AND u.uc_trayecto = :trayecto AND u.uc_periodo = '2' AND u.uc_estado = 1");
    
                    foreach ($clases_origen as $clase) {
                        $stmt_uc_fase2->execute([':doc_id' => $clase['doc_id'], ':trayecto' => $trayecto_destino]);
                        $ucs_posibles = $stmt_uc_fase2->fetchAll(PDO::FETCH_COLUMN);
    
                        if (count($ucs_posibles) === 1) {
                            $nuevas_clases[] = ['uc_id' => $ucs_posibles[0], 'doc_id' => $clase['doc_id'], 'esp_id' => $clase['esp_id'], 'dia' => $clase['dia'], 'hora_inicio' => $clase['hora_inicio'], 'hora_fin' => $clase['hora_fin']];
                        } else {
                            ...
                            $reporte['observaciones'][] = count($ucs_posibles) === 0 ? "Docente <strong>{$nombre_docente}</strong> no tiene UC de Fase 2 para el trayecto {$trayecto_destino}." : "Docente <strong>{$nombre_docente}</strong> tiene múltiples UCs de Fase 2, no se asignó automáticamente.";
                        }
                    }
    ...
    {
        try {
            $stmt = $this->Con()->query("SELECT DISTINCT ts.sec_id, ts.sec_codigo, ts.sec_cantidad, a.ani_anio, a.ani_id FROM tbl_seccion ts JOIN seccion_horario sh ON ts.sec_id = sh.sec_id JOIN tbl_horario th ON sh.hor_id = th.hor_id JOIN tbl_anio a ON ts.ani_id = a.ani_id WHERE th.hor_estado = 1 AND ts.sec_estado = 1 ORDER BY a.ani_anio DESC, ts.sec_codigo");
    ...
    public function ConsultarDetalles($sec_id)
    {
        ...
        try {
            ...
            foreach($schedule_grid_items as $key => $item){
                 if (!empty($item['hora_inicio'])) { $schedule_grid_items[$key]['tur_id'] = $turnosMap[$item['hora_inicio'].":00"] ?? 0; } 
                 else { $schedule_grid_items[$key]['tur_id'] = 0; }
            }
    ...
    public function obtenerUcPorDocente($doc_id) { 
        ...
        try {
            $sql = "SELECT u.uc_id, u.uc_nombre, u.uc_codigo, u.uc_trayecto, u.uc_periodo FROM tbl_uc u INNER JOIN uc_docente ud ON u.uc_id = ud.uc_id WHERE ud.doc_id = :doc_id AND u.uc_estado = 1";
            ...
            if(empty($ucs)){
                return ['data' => [], 'mensaje' => 'El docente no tiene UCs disponibles para la fase actual.'];
            }
    ...
    public function obtenerUnidadesCurriculares() { try { return $this->Con()->query("SELECT uc_id, uc_nombre, uc_codigo, uc_trayecto FROM tbl_uc WHERE uc_estado = 1")->fetchAll(PDO::FETCH_ASSOC); } catch (Exception $e) { return []; } }
    public function obtenerSecciones() { try { return $this->Con()->query("SELECT s.sec_id, s.sec_codigo, s.sec_cantidad, a.ani_anio, s.ani_id FROM tbl_seccion s JOIN tbl_anio a ON s.ani_id = a.ani_id WHERE s.sec_estado = 1 ORDER BY a.ani_anio DESC, s.sec_codigo")->fetchAll(PDO::FETCH_ASSOC); } catch (Exception $e) { return []; } }
    public function obtenerEspacios() { try { return $this->Con()->query("SELECT esp_id, esp_codigo, esp_tipo FROM tbl_espacio WHERE esp_estado = 1")->fetchAll(PDO::FETCH_ASSOC); } catch (Exception $e) { return []; } }
    public function obtenerDocentes() { try { return $this->Con()->query("SELECT doc_id, doc_nombre, doc_apellido FROM tbl_docente WHERE doc_estado = 1")->fetchAll(PDO::FETCH_ASSOC); } catch (Exception $e) { return []; } }
