...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (respuesta) {
        try {
          ...
          fillSelectGenerico("#seccion_principal_id", allSecciones, "sec_id", "sec_codigo", null, "Seleccionar Sección...", "tra_numero", "ani_anio");
  ...
  // ===== INICIO DE MODIFICACIONES: VALIDACIÓN EN TIEMPO REAL =====

  // Se fusiona la carga de UC con la nueva validación de conflicto para el docente.
  $("#modalSeleccionarDocente").on("change", function() {
      const docId = $(this).val();
      const seccionPrincipalId = $("#seccion_principal_id").val();
      const guardarBtn = $("#btnGuardarClase");
  
      $(this).css("border-color", ""); // Resetea su propio estilo
  
      // Carga las UCs del docente (funcionalidad original)
      let traIdParaFiltrar = null;
      if (seccionPrincipalId) {
          const seccionData = allSecciones.find(s => s.sec_id == seccionPrincipalId);
          if (seccionData) traIdParaFiltrar = seccionData.tra_id;
      ...
      // Si no hay docente seleccionado, no validar
          ...
          success: function(respuesta) {
              try {
                  ...
                  if (data.conflicto) {
                      ...
                      // Habilita el botón solo si el otro campo (espacio) tampoco tiene error
                      if ($("#modalSeleccionarEspacio").css('border-color') !== 'rgb(255, 0, 0)') {
                          guardarBtn.prop("disabled", false);
                      }
                  }
  ...
  // Validación en tiempo real para el Espacio
...
function limpiaParaNuevoRegistro() {
  ...
  $("#seccion_principal_id").prop('disabled', false).val(''); 
