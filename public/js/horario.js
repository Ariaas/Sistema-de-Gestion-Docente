function muestraMensaje(tipo, tiempo, titulo, mensaje) {
    if (typeof Swal !== 'undefined' && Swal.fire) {
        ...
        console.log(`[${tipo.toUpperCase()}] ${titulo}: ${mensaje.replace(/<br\/>/g, "\n")}`);
    ...
    if (currentModalMode !== "delete-confirm" && currentModalMode !== "view-only") {
        $(".celda-horario")
            ...
            .on("click", function() {
                ...
                allDocentes.forEach(doc => {
                    docenteSelect.append(`<option value="${doc.doc_id}">${doc.doc_nombre} ${doc.doc_apellido}</option>`);
                });
                ...
                allEspacios.forEach(esp => {
                    espacioSelect.append(`<option value="${esp.esp_id}">${esp.esp_codigo} (${esp.esp_tipo})</option>`);
                });
        ...
        success: function(respuesta) {
            try {
                ...
                if (data.resultado === 'ok' && data.ucs_docente && Array.isArray(data.ucs_docente) && data.ucs_docente.length > 0) {
                    ucSelect.append('<option value="">Seleccionar Unidad Curricular</option>');
                    data.ucs_docente.forEach(uc => {
                        ucSelect.append(`<option value="${uc.uc_id}">${uc.uc_codigo} - ${uc.uc_nombre}</option>`);
                    });
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                try {
                    ...
                    muestraMensaje("error", 4000, "¡ERROR!", "No se pudieron cargar los datos iniciales.");
                }
            },
            error: function() {
                muestraMensaje("error", 4000, "¡ERROR!", "Error de red al cargar los datos de los selects.");
            },
        });
    }
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                try {
                    ...
                    console.error("Error al procesar respuesta de conflicto de docente", e);
                }
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                try {
                    ...
                    console.error("Error al procesar respuesta de conflicto de espacio", e);
                }
            }
        });
    });
    ...
    $("#formularioEntradaHorario").on("submit", function(e) {
        ...
        if (!docente_id || !uc_id || !espacio_id) {
            muestraMensaje("error", 4000, "¡ERROR!", "Debe seleccionar Docente, Unidad Curricular y Espacio.");
            return;
        }
        const franjaInicio24h = currentClickedCell.data("franja-inicio");
        ...
        const cellContent = generarCellContentParaHorarioPrincipal(horarioData);
        currentClickedCell.html(cellContent);
        horarioContenidoGuardado.set(keyCeldaActual, {
            html: cellContent,
            data: horarioData
        });
    ...
    function poblarSelectSecciones(secciones, selector) {
        const seccionSelect = $(selector);
        const opcionesFormateadas = secciones.map(s => {
            const texto = `${s.sec_codigo}-${s.sec_nombre}${s.coh_numero} (Tray. ${s.tra_numero} - Año ${s.ani_anio})`;
            return {
                id: s.sec_id,
                texto: texto
            };
        });
        seccionSelect.empty().append('<option value="">Seleccionar Sección...</option>');
        opcionesFormateadas.forEach(opt => {
            seccionSelect.append(`<option value="${opt.id}">${opt.texto}</option>`);
        });
    }
    ...
    $("#registrar").on("click", function() {
        ...
        if (allSecciones.length === 0) {
            muestraMensaje("warning", 5000, "ATENCIÓN", "Las secciones aún no se han cargado, espere.");
            return;
        }
    ...
    $(document).on('click', '.modificar-grupo-horario', function() {
        ...
        const seccionNombreCompleto = $row.find('td:nth-child(1)').text();
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                try {
                    ...
                    if (lee.resultado === 'ok' && lee.mensaje) {
                        ...
                        muestraMensaje("error", 5000, "¡ERROR!", lee.mensaje || "No se pudieron cargar los detalles del horario.");
                    }
                } catch (e) {
            ...
            error: function() {
                muestraMensaje("error", 5000, "¡ERROR!", "No se pudo contactar al servidor para cargar el horario.");
            }
        });
    });
    ...
    $(document).on('click', '.eliminar-grupo-horario, .ver-grupo-horario', function() {
        ...
        const $row = $(this).closest('tr');
        const seccionNombre = $row.find('td:nth-child(1)').text();
        const faseNum = $row.find('td:nth-child(4)').text();
        limpiaParaNuevoRegistro();
        poblarSelectSecciones(allSecciones, "#seccion_principal_id");
        ...
        $.ajax({
                    ...
                    if (lee.resultado === 'ok' && lee.mensaje) {
                        ...
                        if (Array.isArray(lee.mensaje)) {
                            lee.mensaje.forEach(clase => {
                                const key = `${clase.hora_inicio}-${clase.hora_fin}-${clase.dia}`;
                                horarioContenidoGuardado.set(key, {
                                    html: generarCellContentParaHorarioPrincipal(clase),
                                    data: clase
                                });
                            });
                        }
                        ...
                        muestraMensaje("error", 5000, "¡ERROR!", lee.mensaje || "No se pudieron cargar los detalles.");
                    }
                } catch (e) {
                    muestraMensaje("error", 5000, "¡ERROR!", "Respuesta inválida del servidor.");
                }
            ...
            error: function() {
                muestraMensaje("error", 5000, "¡ERROR!", "No se pudo contactar al servidor.");
            }
        });
    });
                ...
                if (lee.resultado == "consultar_agrupado") {
                    ...
                    if (lee.mensaje && lee.mensaje.length > 0) {
                        $.each(lee.mensaje, function(index, item) {
                            const nombreSeccionCompleto = `${item.sec_codigo}-${item.sec_nombre}${item.coh_numero}`;
                            $("#resultadoconsulta").append(`
                              <tr>
                                <td>${nombreSeccionCompleto}</td>
                                <td>Trayecto ${item.tra_numero || 'N/A'}</td>
                                <td>${item.ani_anio || 'N/A'}</td>
                                <td>${item.fase_numero || 'N/A'}</td>
                                <td>
                                  <button class="btn btn-info btn-sm ver-grupo-horario" data-sec-id="${item.sec_id}" data-fase-id="${item.fase_id}" title="Ver Horario"><i class="bi bi-eye-fill"></i> Ver Horario</button>
