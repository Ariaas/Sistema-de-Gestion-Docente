$(document).ready(function() {
    ...
    if (!hayDatos) {
        ...
        muestraMensaje('info', null, 'Sin Datos para Reportes', 'Actualmente no existen datos de prosecución registrados para generar un reporte.');
    }
    ...
    function displayChart(chartType) {
        if (!currentResponseData) {
            ...
            renderChart({ labels: [], datasets: [] }, 'bar', 'Seleccione los filtros para generar un reporte de prosecución');
            return;
        }
    ...
    function buildChartData(data) {
        ...
        return {
            ...
            datasets: [{
                ...
                label: 'Cantidad de Estudiantes (Prosecución)',
                data: values,
    ...
    function getChartTitle(tipoReporte) {
        ...
        if (tipoReporte === 'general') return 'Total de Prosecución del Periodo Activo';
        if (tipoReporte === 'seccion') return 'Prosecución por Sección Origen';
        if (tipoReporte === 'trayecto') return 'Prosecución por Trayecto Origen';
        return 'Seleccione los filtros para generar un reporte de prosecución';
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.datos && response.datos.length > 0) {
                    ...
                    muestraMensaje('info', null, 'Sin Datos', response.mensaje || 'No se encontraron datos de prosecución para generar el reporte.');
                    renderChart({ labels: [], datasets: [] }, 'bar', 'Seleccione los filtros para generar un reporte de prosecución');
                }
            },
            error: function() {
