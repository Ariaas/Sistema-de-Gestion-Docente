      ...
      try {
        ...
        if (lee.resultado === "consultar") {
          ...
          $.each(lee.mensaje, function (index, item) {
            const esAdmin = item.rol_nombre === 'Administrador';
            $("#resultadoconsulta").append(`
                <tr>
                  <td style="display: none;">${item.rol_id}</td>
                  <td>${item.rol_nombre}</td>
                  <td>
                    <button class="btn btn-icon btn-info" data-codigo="${item.rol_id}" data-nombre="${item.rol_nombre}" ${!PERMISOS.registrar ? 'disabled' : ''}> <img src="public/assets/icons/people.svg" alt="Permisos"></button>
                    <button class="btn btn-icon btn-edit" onclick='pone(this,0)' data-id="${item.rol_id}" data-nombre="${item.rol_nombre}" ${!PERMISOS.modificar || esAdmin ? 'disabled' : ''}><img src="public/assets/icons/edit.svg" alt="Modificar"></button>
                    <button class="btn btn-icon btn-delete" onclick='pone(this,1)' data-id="${item.rol_id}" data-nombre="${item.rol_nombre}" ${!PERMISOS.eliminar || esAdmin ? 'disabled' : ''}><img src="public/assets/icons/trash.svg" alt="Eliminar"></button>
                  </td>
                </tr>
              `);
          });
...
$(document).on("click", ".btn-info[data-codigo]", function () {
  rolSeleccionado = $(this).data("codigo");
  rolSeleccionadoNombre = $(this).data("nombre");
  var datos = new FormData();
  datos.append("accion", "listarPermisos");
  datos.append("rolId", rolSeleccionado);
  enviaAjax(datos);
...
function renderTablaPermisos(modulosDisponibles, permisosAsignados) {
  const tablaBody = $('#tablaPermisos tbody');
  tablaBody.empty();
  ...
  modulosDisponibles.forEach(modulo => {
    ...
    if (modulo.per_modulo === 'reportes') {
      ...
      ['registrar', 'modificar', 'eliminar'].forEach(accion => {
        ...
        const isChecked = permisoAsignado ? 'checked' : '';
        row += `<td class="text-center"><input type="checkbox" class="permiso-check" data-perid="${modulo.per_id}" data-accion="${accion}" ${isChecked} ${esAdmin ? 'disabled' : ''}></td>`;
      });
    }
    row += '</tr>';
  ...
  $('#guardarPermisos').prop('disabled', esAdmin);
  if (esAdmin) {
    muestraMensaje('info', 4000, 'Información', 'Los permisos del rol Administrador no se pueden modificar.');
  }
}